...
def read_and_filter_grn(grn_path, var=0, ci=0, net=None):
    ...
    grn['id'] = grn[['regulator', 'target']].apply(lambda x: '{}_{}'.format(
        x[0], x[1]), axis=1)
    if var > 0:
...
def extract_tf(data, TF, obs_key=None, obs_value=None, my_layer='Ms'):
    ...
    if(obs_key is not None and obs_key in data.obs.keys()):
        ...
        sub = pd.DataFrame(data.layers[my_layer][:,
                           data.var.index.isin(TF[0])])
...
def load_expression(paths):
    adatas = {k.split('_')[0]: sc.read_h5ad(paths[k]) for k in paths if
              '_exp' in k}
    adatas['P2'].obs['cardinal_class'] = adatas['P2'].obs['CellType']
    for tp in adatas:
        adatas[tp] = adatas[tp][:, adatas[tp].var.pct_dropout_by_counts != 100]
        adatas[tp] = adatas[tp][adatas[tp].obs.cardinal_class.isin(['PV',
                                                                   'SST'])]
        sc.tl.rank_genes_groups(adatas[tp], groupby='cardinal_class',
                                rankby_abs=True, pts=True, use_raw=False,
                                layers='normed', method='wilcoxon',
                                n_genes=adatas[tp].shape[1],key_added='DE')
...
def get_de_info(adatas, seltf):
    ...
    for tp in adatas:
        ...
        if(isspmatrix(adatas[tp].layers['normed'])):
            ...
            obs = adatas[tp].layers['normed'].copy()
        obs = pd.DataFrame(obs, columns=gene_ids,
                           index=adatas[tp].obs['cardinal_class'])
        obs_bool = obs.astype(bool)
        fraction_obs = obs_bool.groupby(level=0).sum()
        fraction_obs /= obs_bool.groupby(level=0).count()
        mean_obs = obs.groupby(level=0).mean()
        mean_obs.loc['SST'][mean_obs.loc['SST'] == 0] = 1
        df = np.log2(mean_obs.loc['PV']/mean_obs.loc['SST'])
        mean_obs = pd.DataFrame(df)
        mean_obs.columns = ['l2']
        dedf[tp] = dedf[tp].merge(fraction_obs.T, left_on='names',
                                  right_index=True)
...
def compute_edge_fractions(f_grns):
    ...
    for t in tp:
        report = {}
        network_a, network_b = [x for x in f_grns.keys() if
                                t == x.split('_')[0]]
        ...
        for regulator in set(set(f_net_a.regulator) | set(f_net_b.regulator)):
            ...
            s_edges = set(set(report[regulator][cell_type_b]['edge_id']) &
                          set(report[regulator][cell_type_a]['edge_id']))
